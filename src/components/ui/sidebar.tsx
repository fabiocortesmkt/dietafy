...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
          ...
          <AnimatePresence mode="wait">
            {openMobile && (
              <motion.div
                ...
                exit={{ opacity: 0, x: -16, scale: 0.96, filter: "blur(4px)" }}
                transition={{ duration: 0.3, ease: "easeOut" }}
                className="flex h-full w-full flex-col bg-sidebar/95 overflow-y-auto"
              >
                {children}
              </motion.div>
            )}
  ...
  return (
    ...
    >
      ...
      >
        <motion.div
          initial={{ opacity: 0.96, scale: 0.98, x: -4 }}
  ...
  ({ className, onClick, ...props }, ref) => {
    ...
    return (
      <Button
         ...
         size="icon"
         className={cn(
           "h-11 w-11 md:h-10 md:w-10 rounded-xl border border-sidebar-border/70 bg-background/80 text-foreground transition-colors",
           "hover:bg-muted/40",
           className,
         )}
